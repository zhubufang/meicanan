	...
	<cube-scroll ref="scroll" class="homePage">
		...
		<div class="header flex">
			...
			<div class="ipt flex flex-item">
				...
				<mt-button type="primary" size="small" style="line-height: 40px;height: 40px;background-color:#2F7364;border-radius: 0px;">搜索</mt-button>
			</div>
		</div>
		...
		<div class="store">
			<ul class="flex flex-justify-between m-bottom" >
				<li v-for="(item,index) in saixuan" :key="index" :class="[{'active':isActive==index}]" @click="chooseItem(index)">{{item}}</li>
			</ul>

			<div class="store-list">
				<div style="padding: 10px;" :class="['flex','list', 'm-bottom']" v-for="(item,index) in 6" :key="index" @click="$router.push('/yanglaoyuan')">
